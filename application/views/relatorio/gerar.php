			...
			if(key < 1){
				...
				if(filtro == 'status'){
					...
					if(finalizado != ''){
						...
						novo.children(".status").children(".finalizado").attr("checked", true);	
					}
					if(nao_concluido != ''){
						novo.children(".nao_concluido").attr("checked", true);	
					}
					if(em_andamento != ''){
						novo.children(".em_andamento").attr("checked", true);	
					}
					if(em_espera != ''){
						novo.children(".em_espera").attr("checked", true);	
					}
					if(em_atraso != ''){
						novo.children(".em_atraso").attr("checked", true);	
					}										
				}
				else if(filtro == 'data_agendada'){
					novo.children(".filtrando").find("option[value='"+filtro+"']").attr("selected", true).trigger("change");
...
<div class="form-style">
    <form method="post" id="form_download" target="_self">
            ...
            <button class="btn btn-inverse btn-large" type="submit" id="download">Fazer download para XLS</button>
    </form>

...
// ADD Funcionário
	...
	$("#auto, #auto1").autocomplete("<?php print base_url(); ?>autocomplete/usuariosAutocomplete/",
	{
	  minLength: 2,
	  scrollHeight: 220,
	  selectFirst: false
...
?>
		...
		$("#result_funcionarios").append('<span class="'+classeUser+'"><?php echo $funcionario->nome; ?> <button alt="" class="'+classeUser+' btn btn-danger btn-mini"  style="cursor:pointer;" onclick="remuve(\''+classeUser+'\');">Remover</button></span><br class="'+classeUser+'" />');
<?php		
	}
}
?>
...
<?php		
	...
	foreach ($funcionarios as $funcionario) {
		if($funcionario->idusuario == $atendimento->responsavel){
?>
		var nome = '<?php echo $funcionario->nome; ?>';
		var id   = <?php echo $funcionario->idusuario; ?>;
		var value_imput = nome.replace(/\s/g,'_')+'-'+id;
		...
		$("#result_responsavel").append('<span class="'+classeCliente+'"><?php echo $funcionario->nome; ?> <button alt="" class="'+classeCliente+' btn btn-danger btn-mini" style="cursor:pointer;" onclick="remuve(\''+classeCliente+'\',\'incluir_responsavel\');">Remover</button></span><br class="'+classeCliente+'" />');		
		$("#incluir_responsavel").val("Substituir");
<?php	
		}
	}
...
?>
		...
		$("#result_clientes").append('<span class="'+classeCliente+'"><?php echo $cliente->nome; ?> <button alt="" class="'+classeCliente+' btn btn-danger btn-mini" style="cursor:pointer;" onclick="remuve(\''+classeCliente+'\',\'incluir_cliente\');">Remover</button></span><br class="'+classeCliente+'" />');		
		$("#incluir_cliente").val("Substituir");
<?php	
		}
	}
...
function addResponsavel(){
	$.getJSON("<?php print base_url(); ?>atendimento/getIdNomeResponsavel/"+$("#auto1").val(),{}, function(j){
			if(j != ''){
				var conteudo = $("#form").find('input[name="nome_responsavel"]').length;
				var value_imput = j.nome.replace(/\s/g,'_')+'-'+j.idusuario;
				var idAtendimento = <?php echo $atendimento->idatendimento; ?>;
				var classeUser = 'responsavel_'+j.idusuario;
				...
				if(conteudo == 0){
						...
						$("#result_responsavel").append('<span class="'+classeUser+'">'+j.nome+' <button alt="" class="'+classeUser+' btn btn-danger btn-mini"  style="cursor:pointer;" onclick="remuve(\''+classeUser+'\', \''+btn_add+'\');">Remover</button></span><br class="'+classeUser+'" />');		
						$("#emails_assinatura").val(j.email);
						$("#responsavel").show();
						$("#auto1").val("");
					...
					if(encontrado >= 1){
						...
						$("#result_responsavel").html('<span class="'+classeUser+'">'+j.nome+' <button alt="" class="'+j.idusuario+' btn btn-danger btn-mini"  style="cursor:pointer;" onclick="remuve(\''+classeUser+'\', \''+btn_add+'\');">Remover</button></span><br class="'+classeUser+'" />');		
						$("#emails_assinatura").val(j.email);
						$("#responsavel").show();
						$("#auto1").val("");
						$("#"+btn_add).val("Substituir");
...
<div class="form-style">
    ...
    <input type="hidden" name="idatendimento" id="idatendimento" value="<?php echo $atendimento->idatendimento; ?>" />
	  <fieldset>
        <legend>Responsavel</legend>
            <ul>
                <li id="buscar">
                    <label style="width: 150px;">Adicionar responsavel: </label>
                ...
                <img src="<?php echo base_url('/assets/images/ico_duvida.png'); ?>" class="tooltip-custom" title="Basta digitar qualquer letra do nome do colaborador que logo sugirá uma listagem para selecioná-lo." width="16" height="16" /></li>
        ...
        <legend>Informações principais</legend>
            <ul>
                <li>
                    ...
                    <img src="<?php echo base_url('/assets/images/ico_duvida.png'); ?>" class="tooltip-custom" title="Este campo é livre para você identificar o atendimento do seu jeito. Utilize nomes, números ou qualquer outro padrão já adotado na sua empresa." width="16" height="16" /></li>
                <!--<li>
                    <label>Tag: </label>
                ...
                <img src="<?php echo base_url('/assets/images/ico_duvida.png'); ?>" class="tooltip-custom" title="Coloque aqui tudo que possa orientar a sua equipe externa na execução do serviço. Podem ser instruções, lembretes, checklists entre outros." width="16" height="16" /></li>  
                <li>
                ...
                <img src="<?php echo base_url('/assets/images/ico_duvida.png'); ?>" class="tooltip-custom" title="Selecione no calendário o dia pretendido para o agendamento ou digite a data no formato dd/mm/aaaa. " width="16" height="16" /></li> 
                <li>
                ...
                <img src="<?php echo base_url('/assets/images/ico_duvida.png'); ?>" class="tooltip-custom" title="Digite a hora de início do atendimento no formato hh:mm. " width="16" height="16" /></li> 
                <li>
                ...
                <img src="<?php echo base_url('/assets/images/ico_duvida.png'); ?>" class="tooltip-custom" title="Digite no formato hh:mm o tempo estimado para este atendimento. " width="16" height="16" /></li>            
	  ...
	  <fieldset>
        <legend>Equipe Externa</legend>
            <ul>
                ...
                <img src="<?php echo base_url('/assets/images/ico_duvida.png'); ?>" class="tooltip-custom" title="Basta digitar qualquer letra do nome do colaborador que logo sugirá uma listagem para selecioná-lo. Você pode incluir quantos forem necessário para compor a equipe externa deste atendimento." width="16" height="16" /></li>
 ...
 <fieldset>
        ...
        <img src="<?php echo base_url('/assets/images/ico_duvida.png'); ?>" class="tooltip-custom" title="Você pode editar estas informações, mas atenção, elas valerão apenas para este atendimento. Qualquer alteração feita aqui não modificará o cadastro original de localização do cliente." width="16" height="16" /></legend>
<ul>
          <li>
                ...
                <img src="<?php echo base_url('/assets/images/ico_duvida.png'); ?>" class="tooltip-custom" title="Você pode complementar com dicas temporárias para ajudar a chegada ao local, principalmente em situações atípicas. Alternativas de transportes e melhores rotas são de grande utilidade." width="16" height="16" /></li>            
      </ul>
      </fieldset>      
	  ...
	  <fieldset>
        <legend>Controle de Status</legend>
            <ul>
                ...
                <img src="<?php echo base_url('/assets/images/ico_duvida.png'); ?>" class="tooltip-custom" title="Caso opte por 'Sim', o atendimento será assinalado com uma estrela." width="16" height="16" /></li>
                <li>
                    <label>Status: </label>
                ...
                <img src="<?php echo base_url('/assets/images/ico_duvida.png'); ?>" class="tooltip-custom" title="Você tem a liberdade de escolher o status que mais se adequar a este atendimento." width="16" height="16" /></li> 
                <li>
                ...
                <li class="mostra-contrasenha">
					...
					<img src="<?php echo base_url('/assets/images/ico_duvida.png'); ?>" class="tooltip-custom" title="Aqui você cadastra a assinatura que é enviada diretamente para o email registrado como contato do cliente. Na finalização do atendimento, o cliente deverá digitá-la no campo 'Assinatura' na tela de checkout no aplicativo do celular da equipe externa." width="16" height="16" />
				</li>  
				<li class="mostra-contrasenha">
                    <label>E-mail: </label>
	...
	<fieldset>
		...
		<ul> 
			   <li>
					...
					<input type="radio" name="novo_atendimento" value="nao" <?php echo set_radio('novo_atendimento', 'nao'); ?>> Não&nbsp;&nbsp;<span class="mostra-contrasenha"><img src="<?php echo base_url('/assets/images/ico_duvida.png'); ?>" class="tooltip-custom" title="Ao marcar esta opção como 'Sim', a sua equipe externa receberá uma notificação no aplicativo do celular ao sincronizá-lo com o sistema." width="16" height="16" /></span>

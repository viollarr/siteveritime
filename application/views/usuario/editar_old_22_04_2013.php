...
?>
		...
		if(<?php echo $_POST['idpermissao']; ?> == 3){
			...
			?>		
					...
					$("#result_users").append('<span class="'+classeUser+'"><?php echo $nome_funcionario; ?> <img src="<?php echo base_url('assets/images/ico_cross.png'); ?>" alt="" class="'+classeUser+'"  style="cursor:pointer;" onclick="remuve(\''+classeUser+'\',1);"/></span><br class="'+classeUser+'" />');
			<?php		
				}
			}
			?>
...
?>
			...
			$("#result_users").append('<span class="'+classeUser+'">'+nome+' <img src="<?php echo base_url('assets/images/ico_cross.png'); ?>" alt="" class="'+classeUser+'"  style="cursor:pointer;" onclick="remuve(\''+classeUser+'\',1);"/></span><br class="'+classeUser+'" />');
<?php	
		}
	}
?>
...
function addRelacionamento(){
	$.getJSON("<?php print base_url(); ?>atendimento/getIdNomeUsuario/"+$("#users").val(),{}, function(j){
			...
			if(j != ''){
				...
				if(conteudo == 0){
						$("#form").append('<input type="hidden" name="funcionarios_alocados[]" value="'+value_imput+'" class="'+classeUser+'" />');
						$("#result_users").append('<span class="'+classeUser+'">'+j.nome+' <img src="<?php echo base_url('assets/images/ico_cross.png'); ?>" alt="" class="'+classeUser+'"  style="cursor:pointer;" onclick="remuve(\''+classeUser+'\');"/></span><br class="'+classeUser+'" />');
						$("#user_escolhido").show();
						$("#users").val("");
				}
				else{
					...
					if(encontrado >= 1){
						...
						$("#form").append('<input type="hidden" name="funcionarios_alocados[]" value="'+value_imput+'" class="'+classeUser+'" />');
						$("#result_users").append('<span class="'+classeUser+'">'+j.nome+' <img src="<?php echo base_url('assets/images/ico_cross.png'); ?>" alt="" class="'+j.idusuario+'"  style="cursor:pointer;" onclick="remuve(\''+classeUser+'\');"/></span><br class="'+classeUser+'" />');
						$("#user_escolhido").show();
						$("#users").val("");
					}
				}

...
class usuario extends CI_Controller {
    ...
    function visualizar() {
		...
		if (!empty($usuario)) {
			...
			if($this->session->userdata('usuario')->idpermissao == 2){
				$dados["administradores"] = $this->usuario_model->get_usuarios_adm()->result();
			}else if($this->session->userdata('usuario')->idpermissao == 3){
				$dados["gestores"] = $this->usuario_model->get_usuarios_adm()->result();
			}			
    ...
    */
    ...
    private function validar_form() {
		...
		$this->form_validation->set_rules('notif_email', 'notif_email');
		$this->form_validation->set_rules('notif_sms', 'notif_sms');

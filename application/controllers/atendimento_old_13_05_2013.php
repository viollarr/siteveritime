...
*/            // 2º CHAMAR MÉTODO POST() DO MODELO.
    ...
    public function editar() {
        ...
        if (!empty($atendimento)) {
			...
			$dados["funcionarios"] = $this->usuario_model->get_usuarios_by_empresa();
			$dados["clientes"] = $this->cliente_model->get_clientes_by_empresa();
...
public function update_reagendar() {
     ...
     * Retorna o id e nome do usuário para preenchimento do autocomplete.
     */
	public function getIdNomeResponsavel($nomeUser){
    ...
    public function valida_responsavel() {
        ...
        if (!empty($campo_responsavel)) {
			$this->load->model("usuario_model");
			$retorno = json_decode($this->usuario_model->get_by_id_nome($campo_responsavel));
            ...
            if (empty($nome_responsavel)) {
				$this->form_validation->set_message('valida_responsavel', "O campo <strong>Responsavel</strong> é necessário.");
				return false;
            } else {
                return true;
            }

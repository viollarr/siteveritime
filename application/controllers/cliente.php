...
class Cliente extends CI_Controller {
	...
	public function busca($offset = 0) {
        ...
        if (!empty($parms['campo_busca'])) {
            $dados['clientes'] = $this->cliente_model->lista($parms)->result();
    ...
    public function insert() {
        ...
        if ($this->validar_form()) {
            ...
            if ($this->cliente_model->insert() > 0) {
				// Salvar contato principal
				$this->cliente_model->salvar_contato_principal();
				
				// Salvar os contatos.
                $this->cliente_model->salvar_contatos_cliente();
    ...
    public function editar() {
        ...
        if (!empty($cliente)) {
			...
			//segundo campo é para determinar se é ou não o contato principal
			$dados['contato_principal'] = $this->cliente_model->contatos_adicionais($idcliente, "sim")->row();
            ...
            // Pegando os contatos.
            $dados['contatos'] = $this->cliente_model->contatos_adicionais($idcliente)->result();
    ...
    public function update() {
        ...
        if (!empty($idcliente)) {
            ...
            if ($this->validar_form()) {
                ...
                if ($atualizado) {
                    // Salvar contato principal
					$this->cliente_model->atualizar_contato_principal($idcliente);
					
                    // Salvar os contatos.
                    $this->cliente_model->salvar_contatos_cliente_edicao($idcliente);
    ...
    private function validar_form() {
		...
		$this->form_validation->set_rules('ramal');
        ...
        $this->form_validation->set_rules('contatos', 'Contatos', 'callback_verificar_contatos');
        $this->form_validation->set_rules('endereco_filiais', 'Endereços das Filiais', 'callback_verificar_enderecos_filiais');

        return $this->form_validation->run();
    }

    public function verificar_contatos() {
        $this->load->model("cliente_model");

        // Pegar endereços dos contatos preenchidos até o momento no formulário.
        $contatos = $this->cliente_model->form_contatos();

        // Se não foi preenchido nenhum form_contatos.
        if (empty($contatos))
            return true;
        ...
        foreach ($contatos as $contato) {
            if (empty($contato->contato_responsavel)) {
                $erros[] = "<li>O <strong>responsavel do contato {$i}</strong> deve ser preenchido.</li>";
            }

            if (empty($contato->contato_email)) {
                $erros[] = "<li>O <strong>email do contato {$i}</strong> deve ser preenchido.</li>";
            }else{
				if(!filter_var($contato->contato_email, FILTER_VALIDATE_EMAIL)){ 
					$erros[] = "<li>O <strong>email do contato {$i}</strong> deve ser um email valido.</li>";
				}
			}
        ...
        if (empty($erros)) {
            ...
            $this->form_validation->set_message('verificar_contatos', "Falta alguma informação no cadastro dos contatos: <ul>" . implode("", $erros) . "</ul>");

...
class Atendimento extends CI_Controller {
	...
	public function configura_atendimento_info($atendimento = ""){
		if(!empty($atendimento)){
			...
			$data_hora_checkin = $atendimento->data_hora_checkin;
			$data_hora_checkout = $atendimento->data_hora_checkout;
			...
			$atendimento->data_hora_checkin = (!empty($atendimento->data_hora_checkin)) ? date('d/m/Y - H:i', strtotime($atendimento->data_hora_checkin)).'h' : "Nenhum check-in realizado";
			
			$atendimento->data_hora_checkout = (!empty($atendimento->data_hora_checkout)) ? date('d/m/Y - H:i', strtotime($atendimento->data_hora_checkout)).'h' : "Nenhum check-out realizado";
			
			
			if((!empty($atendimento->latitude_checkin)) && (!empty($atendimento->longitude_checkin))){
				$details_url = "http://maps.googleapis.com/maps/api/geocode/json?latlng=".$atendimento->latitude_checkin.",".$atendimento->longitude_checkin."&sensor=false";
				$ch = curl_init();
				curl_setopt($ch, CURLOPT_URL, $details_url);
				curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
				$geoloc = json_decode(curl_exec($ch), true);
				$atendimento->endereco_completo_checkin = $geoloc['results'][0]['formatted_address'];
			}
					
			$endereco_completo_checkout = "";			
			if((!empty($atendimento->latitude_checkout)) && (!empty($atendimento->longitude_checkout))){
				$details_url = "http://maps.googleapis.com/maps/api/geocode/json?latlng=".$atendimento->latitude_checkout.",".$atendimento->longitude_checkout."&sensor=false";
				$ch = curl_init();
				curl_setopt($ch, CURLOPT_URL, $details_url);
				curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
				$geoloc = json_decode(curl_exec($ch), true);
				$atendimento->endereco_completo_checkout = $geoloc['results'][0]['formatted_address'];
			}
			
			if((!empty($data_hora_checkin)) && (!empty($data_hora_checkout))){	
				$query = $this->db->query("SELECT TIMEDIFF('$data_hora_checkout', '$data_hora_checkin') duracao")->row();
				$atendimento->duracao = $query->duracao;	
				//pr("duracao", $atendimento->duracao);
			}
		}

...
################################################# CONSULTAS APP #######################################################################
	...
	function updateApp($dataTime, $hora, $latitude, $lat, $longitude, $long, $idUsuario, $idAtendimento, $status=NULL, $txtStatus=NULL, $textoCheck, $idRelacionamento=""){
		if($status){
			$checkoutFinal = $this->db->query("SELECT COUNT(idusuario_atendimento) FROM " . $this->db->dbprefix . "usuario_atendimento WHERE ((idatendimento = '".(int)$idAtendimento."' AND data_hora_checkout IS NULL) AND (idusuario <> '".(int)$idUsuario."') )")->result();
			if($checkoutFinal == 0){

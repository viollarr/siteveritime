...
class Atendimento_model extends CI_Model {
    ...
    function post($latitude = '', $longitude = '') {
        if ($this->input->server('REQUEST_METHOD') == 'POST') {
			...
			if(!empty($campo_responsavel)){
				...
				$responsavel = json_decode($this->usuario_model->get_by_id_nome($campo_responsavel));
				$this->responsavel = $responsavel->idusuario;
			}else{
			...
			if(!empty($campo_cliente)){
				...
				$this->endereco = $idcliente->endereco;
				$this->endereco_numero = $idcliente->endereco_numero;
				$this->endereco_complemento = $idcliente->endereco_complemento;
				$this->bairro = $idcliente->bairro;
				$this->idestado = $idcliente->idestado;
				$this->idcidade = $idcliente->idcidade;
				$this->pontos_referencias = $idcliente->pontos_referencias;
			}else{
	...
	function get_atendimentos($params = array()){	
		...
		if($this->session->userdata('usuario')->idpermissao == 1){
			...
			$ids = $this->db->query("SELECT DISTINCT(ua.idatendimento) FROM ".$this->db->dbprefix. "usuario_atendimento ua LEFT JOIN ".$this->db->dbprefix. "usuario_relacionamento ur ON ua.idusuario = ur.id_usuario, ".$this->db->dbprefix. "atendimento a WHERE (ua.idatendimento = a.idatendimento) AND ((a.criado_por = 1) OR ((ua.idusuario = ".$this->session->userdata('usuario')->idusuario.") OR (ur.id_adm_gerente = ".$this->session->userdata('usuario')->idusuario." and ur.id_usuario = ua.idusuario)))")->result();
		...
		$this->db->join("vt_cliente cli", "cli.idcliente = at.idcliente");
		$this->db->join("vt_usuario_atendimento usu_at", "usu_at.idatendimento = at.idatendimento");
		$this->db->join("vt_usuario usu", "usu.idusuario = usu_at.idusuario");				
...
//insert
	...
	*/
     ...
     * @return objeto atendimento 
     */
	function get_atendimentos_mapa($tipo = ""){
		$idempresa = $this->session->userdata('usuario')->idempresa;
		if(!empty($idempresa)){
			$this->db->select('at.idatendimento, at.titulo, at.latitude, at.longitude, at.data_agendada, at.status');
			$this->db->select('(SELECT date(ua.data_hora_checkout) FROM '.$this->db->dbprefix.'usuario_atendimento as ua WHERE ua.data_hora_checkout is not null AND ua.idatendimento = at.idatendimento LIMIT 1) as ultimo_checkout');
			...
			if($tipo == "checkout"){
				$this->db->having("ultimo_checkout IS NOT NULL");
			}else{
				$this->db->having("ultimo_checkout IS NULL");
			}
			$this->db->having("((at.data_agendada = '".date("Y-m-d")."') or (date(ultimo_checkout) = '".date("Y-m-d")."') or (at.status = 'em_andamento'))");
			$this->db->order_by('at.titulo');
			$query = $this->db->get();
	...
	function get_funcionario_checkin(){
		...
		if(!empty($idempresa)){
			...
			$this->db->where("data_hora_checkin =(SELECT MAX(ua2.data_hora_checkin )FROM vt_usuario_atendimento as ua2 WHERE ua2.idusuario = ua.idusuario ) AND date(ua.data_hora_checkin) = '".date("Y-m-d")."'");
			$this->db->where("at.idempresa", $idempresa);
			$query = $this->db->get();

...
class Atendimento_model extends CI_Model {
	...
	function get_atendimentos($params = array()){	
	    ...
	    $this->db->select("at.idatendimento,at.idempresa,at.titulo,at.prioridade, at.data_agendada,at.hora_agendada,at.endereco, at.endereco_numero, at.endereco_complemento,at.bairro, at.status,cli.idcliente, cli.nome AS nome_cliente, GROUP_CONCAT(usu.nome SEPARATOR ', ') as nome_usuario", false);
		...
		if(!empty($campo_dias)){
			$datahoje = date("Y-m-d"); 
			$data_mais_1dia = date('Y-m-d', strtotime("+1 days"));
			$data_mais_6dias = date('Y-m-d', strtotime("+6 days"));
			if ($campo_dias==1){
				$where_campo_dias = "(at.data_agendada = '$datahoje')";	
			}else if ($campo_dias==2){
				$where_campo_dias = "(at.data_agendada >= '$datahoje' AND at.data_agendada<='$data_mais_1dia' )";	
			}else if ($campo_dias==7){
				$where_campo_dias = "(at.data_agendada >= '$datahoje' AND at.data_agendada<='$data_mais_6dias')";	
			}
			$this->db->where($where_campo_dias);
		}
		
		if(!empty($periodo_data_range)){
			if ( (!empty($from)) and (!empty($to)) ){
				$from = fdata($from, "-");
				$to = fdata($to, "-");
				$where_campo_periodo = "(at.data_agendada >= '$from' AND at.data_agendada<='$to')";	
				$this->db->where($where_campo_periodo);
			}
			if ( (!empty($from)) and (empty($to)) ){
				$from = fdata($from, "-");
				$where_campo_periodo = "(at.data_agendada >= '$from')";	
				$this->db->where($where_campo_periodo);
			}
			if ( (empty($from)) and (!empty($to)) ){
				$to = fdata($to, "-");
				$where_campo_periodo = "(at.data_agendada <= '$to')";	
				$this->db->where($where_campo_periodo);
			}			
		...
		//pexit("atendimentos", $this->db->last_query());
	...
	function get_atendimento($idatendimento = ""){	
		$atendimento = array();
		if(!empty($idatendimento)){			
			$this->db->select("at.idatendimento, at.titulo as atendimento, at.prioridade, at.descricao, at.data_agendada, at.hora_agendada, at.tempo_estimado, at.endereco, at.endereco_numero, at.endereco_complemento, at.bairro, at.status, cli.nome AS nome_cliente, es.nome AS nomeestado, es.sigla AS uf, ci.nome AS nomecidade, GROUP_CONCAT(usu.nome ) as nome_usuario");
			$this->db->select('ccPrincipal.contato_responsavel as nome_contato_principal, ccPrincipal.contato_telefone telefone_contato_principal, ccPrincipal.contato_ramal ramal_contato_principal, ccPrincipal.contato_celular celular_contato_principal');
			$this->db->select('ccResponsavel.contato_responsavel as nome_contato_responsavel, ccResponsavel.contato_telefone telefone_contato_responsavel, ccResponsavel.contato_ramal ramal_contato_responsavel, ccResponsavel.contato_celular celular_contato_responsavel');
			//$this->db->select('usu_at2.data_hora_checkin, usu_at2.latitude_checkin, usu_at2.longitude_checkin');
			//$this->db->select('usu_at3.data_hora_checkout, usu_at3.latitude_checkout, usu_at3.longitude_checkout');
			...
			//$this->db->join($this->db->dbprefix."usuario_atendimento usu_at2", "usu_at2.idatendimento = at.idatendimento and usu_at2.data_hora_checkin = (SELECT min(data_hora_checkin) FROM vt_usuario_atendimento WHERE idatendimento = at.idatendimento GROUP BY idatendimento)", "left");
			//$this->db->join($this->db->dbprefix."usuario_atendimento usu_at3", "usu_at3.idatendimento = at.idatendimento and usu_at3.data_hora_checkout = (SELECT min(data_hora_checkout) FROM vt_usuario_atendimento WHERE idatendimento = at.idatendimento GROUP BY idatendimento)", "left");
			$this->db->join($this->db->dbprefix."usuario usu", "usu.idusuario = usu_at.idusuario", "left");				
			$this->db->join($this->db->dbprefix."cliente_contato ccPrincipal", "ccPrincipal.idcliente = cli.idcliente", "left");		
			$this->db->join($this->db->dbprefix."cliente_contato ccResponsavel", "ccResponsavel.idcliente_contato = at.responsavel", "left");
			...
			$this->db->where("at.idempresa = ".$this->session->userdata('usuario')->idempresa);
			$this->db->where("at.idatendimento", $idatendimento);	
			$this->db->where("ccPrincipal.contato_principal", 'sim');
			$this->db->group_by('at.idatendimento');
			$atendimento =  $this->db->get()->row();
			//pexit("atendimento", $this->db->last_query());
			//print $this->db->last_query();
		}
	...
	*/
	...
	function get_atendimentos_mapa($status = ""){
		...
		if(!empty($idempresa)){
			...
			$this->db->select('at.data_agendada, at.hora_agendada, at.tempo_estimado, at.endereco, at.endereco_numero, at.endereco_complemento, at.bairro');
			...
			$this->db->select('(SELECT min(ua.data_hora_checkin) FROM '.$this->db->dbprefix.'usuario_atendimento as ua WHERE ua.idatendimento = at.idatendimento ORDER BY ua.data_hora_checkin ASC LIMIT 1) as ultimo_checkin');
			$this->db->select('(SELECT max(ua.data_hora_checkout) FROM '.$this->db->dbprefix.'usuario_atendimento AS ua WHERE ua.idatendimento = at.idatendimento HAVING count(ua.data_hora_checkout) = count(ua.idusuario_atendimento)) as ultimo_checkout');
			//$this->db->select('(SELECT ua.data_hora_checkout FROM '.$this->db->dbprefix.'usuario_atendimento as ua WHERE ua.data_hora_checkout is not null AND ua.idatendimento = at.idatendimento ORDER BY ua.data_hora_checkout ASC LIMIT 1) as ultimo_checkout');
			$this->db->select('ccPrincipal.contato_responsavel as nome_contato_principal, ccPrincipal.contato_telefone telefone_contato_principal, ccPrincipal.contato_ramal ramal_contato_principal, ccPrincipal.contato_celular celular_contato_principal');
			$this->db->select('ccResponsavel.contato_responsavel as nome_contato_responsavel, ccResponsavel.contato_telefone telefone_contato_responsavel, ccResponsavel.contato_ramal ramal_contato_responsavel, ccResponsavel.contato_celular celular_contato_responsavel');
			$this->db->from($this->db->dbprefix . 'atendimento at');
			$this->db->join($this->db->dbprefix."cliente cli", "cli.idcliente = at.idcliente", "left");
	...
	function get_atendimentos_nome(){
		$idempresa = $this->session->userdata('usuario')->idempresa;
		if(!empty($idempresa)){
			...
			$this->db->select('at.titulo, at.data_agendada, at.status');
			$this->db->select('(SELECT ua.data_hora_checkout FROM '.$this->db->dbprefix.'usuario_atendimento as ua WHERE ua.data_hora_checkout is not null AND ua.idatendimento = at.idatendimento ORDER BY ua.data_hora_checkout ASC LIMIT 1) as ultimo_checkout');
			$this->db->from($this->db->dbprefix . 'atendimento at');
			$this->db->where("at.idempresa", $idempresa);
			$this->db->where("at.latitude IS NOT NULL");
	...
	function get_funcionario_checkin(){
		...
		if(!empty($idempresa)){
			...
			$this->db->select('ua.*, u.nome as nome_usuario, u.latitude_usuario, u.longitude_usuario, at.titulo as titulo_atendimento');
			$this->db->select('ua.latitude_checkin, ua.longitude_checkin, ua.latitude_checkout, ua.longitude_checkout');
	...
	//utilizo para buscar os dados dos usuários vinculados a um atendimento
	function usuarios_atendimento($idatendimento = 0){
		if(!empty($idatendimento)){
			$this->db->select('usu.nome, ua.data_hora_checkin, ua.data_hora_checkout, ua.latitude_checkin, ua.longitude_checkin, ua.latitude_checkout, ua.longitude_checkout, ua.status, ua.visita');
			$this->db->select('uao.observacao');
			$this->db->select('TIMEDIFF(ua.data_hora_checkout, ua.data_hora_checkin) duracao', false);
			$this->db->from($this->db->dbprefix . 'usuario_atendimento as ua, '.$this->db->dbprefix.'usuario as usu');
			$this->db->join($this->db->dbprefix . "atendimento_observacao uao", "uao.id_usuario = usu.idusuario AND uao.id_atendimento = '$idatendimento'", "left");
			$this->db->where("ua.idusuario = usu.idusuario");
			$this->db->where("ua.idatendimento", $idatendimento);
			//$this->db->where("u.ativo", "sim");
			$query = $this->db->get();			
			return $query;
		}
...
################################################# CONSULTAS APP #######################################################################
	function consultApp($id_usuario){
		...
		if($id_usuario > 0){
			...
			SELECT 
					...
					(SELECT celular FROM " . $this->db->dbprefix . "usuario WHERE idusuario = atendimento.criado_por LIMIT 1) as telefoneCriado,
					cidade.nome AS cidade,
					estado.sigla AS estado,
					cliente.idcliente, 
					atendimento.bairro, 
					usatendimento.status, 
					usatendimento.idusuario_atendimento,
				...
				ORDER BY 
					...
					atendimento.data_agendada DESC
	...
	function consultAppGrupos($id_usuario){
		...
		if($id_usuario > 0){
			...
			SELECT
				...
				COUNT(a.idatendimento) AS qtde
			FROM
				" . $this->db->dbprefix . "usuario u
				INNER JOIN " . $this->db->dbprefix . "usuario_atendimento ua ON ua.idusuario = u.idusuario
				INNER JOIN " . $this->db->dbprefix . "atendimento a ON a.idatendimento = ua.idatendimento
			WHERE
	...
	function emailCriador($idAtendimento){
		$idAtendimento = (int)$idAtendimento;
		
		if($idAtendimento > 0){
			$pega_email_criador = $this->db->query("
				SELECT 
					us.email 
				FROM
				" . $this->db->dbprefix . "atendimento at
				INNER JOIN " . $this->db->dbprefix . "usuario us ON at.criado_por = us.idusuario
				WHERE
					at.idatendimento = '".$idAtendimento."'
				LIMIT 1
			"
	...
	function updateApp($dataTime, $hora, $latitude, $lat, $longitude, $long, $idUsuario, $idAtendimento, $status=NULL, $txtStatus=NULL, $textoCheck, $idRelacionamento=""){
		if($status){
			$checkoutFinal = $this->db->query("SELECT COUNT(idusuario_atendimento) as qtde FROM " . $this->db->dbprefix . "usuario_atendimento WHERE ((idatendimento = '".(int)$idAtendimento."' AND data_hora_checkout IS NULL) AND (idusuario <> '".(int)$idUsuario."') )")->row();
			if($checkoutFinal->qtde == 0){
		...
		if(!empty($idRelacionamento)){
			$this->db->where("idusuario_atendimento", (int) $idRelacionamento);
		}
	...
	function insertComentApp($idUsuario, $idAtendimento, $observacao, $dataObservacao, $idRelacionamento=""){
		$data = new stdClass();
		$data->id_usuario = (int)$idUsuario;
		$data->id_atendimento = (int)$idAtendimento;
		$data->observacao = $observacao;
		$data->data_observacao = $dataObservacao;
		if(!empty($idRelacionamento)){
			$data->idusuario_atendimento = $idRelacionamento;
		}		

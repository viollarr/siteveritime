...
class Usuario_model extends CI_Model {
    ...
    function post() {
        if ($this->input->server('REQUEST_METHOD') == 'POST') {
            ...
            $this->login = $this->input->post('email', TRUE);
    ...
    function lista($params = array()) {
	    ...
	    $this->db->select('u.idusuario, u.nome, u.email, u.celular, u.idpermissao, u.ativo, permissao.nome as perfil');
    ...
    function get_idusuario($login, $senha) {
        ...
        if (!empty($login) && !empty($senha)):
            $usuario = $this->db->query("SELECT idusuario, ativo FROM  " . $this->db->dbprefix . "usuario WHERE login = '{$login}' AND senha = '{$senha}' LIMIT 1")->row();
            if (!empty($usuario))
                return $usuario;
            else
                return -1;
    ...
    function get_usuarios_by_empresa() {
        ...
        $usuario_dadaos = $this->db->query("SELECT idusuario, nome FROM " . $this->db->dbprefix . "usuario WHERE idempresa = {$idempresa} AND ativo = 'sim' ORDER BY idpermissao ASC ")->result();
        return $usuario;
    }
    ...
    function get_by_id_nome($usuarioNome) {
        ...
        if (strlen($usuarioNome) > 0){
			$users = $this->db->query("SELECT idusuario, nome FROM " . $this->db->dbprefix . "usuario WHERE nome = '{$usuarioNome}' LIMIT 1")->row();

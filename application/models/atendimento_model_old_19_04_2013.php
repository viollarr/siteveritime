	...
	function get_atendimentos($campo_busca='', $exibe_finalizados=''){
		...
		if($this->session->userdata('usuario')->idpermissao == 1){
			...
			if(!empty($ids)){
				...
				$condicao .= "AND at.idatendimento IN ({$ids_permitidos})";		
			}
			else{
...
//insert
    ...
    function delete($idregistro) {
        ...
        if ($idregistro > 0) {
			return $this->db->delete('vt_atendimento', array('idatendimento' => $idregistro)) ? TRUE : FALSE ; 
        }
        else
            return FALSE;
    }   	
	...
	*/
	...
	function marcar_notificacao($id_atendimento){
		$marcar = $this->db->query("UPDATE " . $this->db->dbprefix . "atendimentos_novos SET data_cadastro = NOW(), notificar = 'sim' WHERE id_atendimento = '{$id_atendimento}'");
		if($marcar)
			return true;
		else
			return false;
